...
describe('BotResponseMultiplexer', () => {
    it('routes messages with no prefix to the default topic', async () => {
        ...
        const [published, publishedResult] = promise();
    ...
    it('treats unknown tags as content', async () => {
        const multiplexer = new BotResponseMultiplexer();
        const [published, publishedResult] = promise();
        multiplexer.sub(BotResponseMultiplexer.DEFAULT_TOPIC, new BufferedBotResponseSubscriber(content => {
    ...
    it('routes messages to subscribers', async () => {
        const multiplexer = new BotResponseMultiplexer();
        multiplexer.sub('cashier', new BufferedBotResponseSubscriber(content => {
            assert.deepStrictEqual(content, 'one double tall latte please\nand a donut\n');
            return Promise.resolve();
        }));
        multiplexer.sub('barista', new BufferedBotResponseSubscriber(content => {
 ...
 */
class BufferedBotResponseSubscriber {
    callback;
    buffer_ = [];
    /**
...
//# sourceMappingURL=bot-response-multiplexer.test.js.map