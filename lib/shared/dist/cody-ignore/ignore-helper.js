...
export class IgnoreHelper {
     ...
     */
    setIgnoreFiles(workspaceRoot, ignoreFiles) {
        ...
        for (const ignoreFile of ignoreFiles) {
            ...
            const relativeFolderUriPath = pathFunctionsForURI(workspaceRoot).relative(workspaceRoot.path, effectiveDir.path);
    ...
    clearIgnoreFiles(workspaceRoot) {
    ...
    findWorkspaceRoot(file) {
        const candidates = Array.from(this.workspaceIgnores.keys()).filter(workspaceRoot => uriHasPrefix(file, URI.parse(workspaceRoot), isWindows()));
 ...
 */
export function ignoreFileEffectiveDirectory(ignoreFile) {
    return Utils.joinPath(ignoreFile, '..', '..');
}
//# sourceMappingURL=ignore-helper.js.map