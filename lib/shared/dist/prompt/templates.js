...
{text}
`;
export function populateCodeContextTemplate(code, fileUri, repoName, type = 'chat') {
    const template = type === 'edit'
...
{text}
\`\`\``;
export function populatePreciseCodeContextTemplate(symbol, fileUri, code) {
...
const MARKDOWN_CONTEXT_TEMPLATE_WITH_REPO = 'Use the following text from file `{filePath}` in repository `{repoName}`:\n{text}';
export function populateMarkdownContextTemplate(markdown, fileUri, repoName) {
    return (repoName
...
const CURRENT_EDITOR_CODE_TEMPLATE_WITH_REPO = 'I have the `{filePath}` file from the repository `{repoName}` opened in my editor. ';
export function populateCurrentEditorContextTemplate(code, fileUri, repoName) {
    const context = languageFromFilename(fileUri) === ProgrammingLanguage.Markdown
    ...
    return ((repoName
        ...
        : CURRENT_EDITOR_CODE_TEMPLATE).replaceAll('{filePath}', displayPath(fileUri)) + context);
}
const CURRENT_EDITOR_SELECTED_CODE_TEMPLATE = 'Here is the selected {languageID} code from file path `{filePath}`: ';
const CURRENT_EDITOR_SELECTED_CODE_TEMPLATE_WITH_REPO = 'Here is the selected code from file `{filePath}` in the {repoName} repository, written in {languageID}: ';
export function populateCurrentEditorSelectedContextTemplate(code, fileUri, repoName) {
    const context = languageFromFilename(fileUri) === ProgrammingLanguage.Markdown
        ? populateMarkdownContextTemplate(code, fileUri, repoName)
        : populateCodeContextTemplate(code, fileUri, repoName);
    return ((repoName
...
{code}
`;
export function populateCurrentEditorDiagnosticsTemplate({ message, type, text }, fileUri) {
...
export function populateTerminalOutputContextTemplate(output) {
...
export function populateCurrentSelectedCodeContextTemplate(code, fileUri, repoName) {
    return (repoName
...
export function populateListOfFilesContextTemplate(fileList, fileUri) {
    return ((fileUri
        ? DIRECTORY_FILE_LIST_TEMPLATE.replace('{fileName}', displayPath(fileUri))
        : ROOT_DIRECTORY_FILE_LIST_TEMPLATE) + fileList);
}
export function populateContextTemplateFromText(templateText, content, fileUri) {
...
export function populateImportListContextTemplate(importList, fileUri) {
...
`;
export function populateCodeGenerationContextTemplate(precedingText, followingText, fileUri, tag) {
