...
export class GraphQLTelemetryExporter {
    ...
    exportMode;
    legacySiteIdentification;
    constructor(client, anonymousUserID, 
    /**
    ...
    async exportEvents(events) {
        ...
        if (this.exportMode === 'legacy') {
            ...
            const resultOrError = await Promise.all(events.map(event => this.client.logEvent({
                ...
                publicArgument: () => event.parameters.metadata?.reduce((acc, curr) => ({
            ...
            if (isError(resultOrError)) {
                logError('GraphQLTelemetryExporter', 'Error exporting telemetry events as legacy event logs:', resultOrError, {
 ...
 */
export function handleExportModeTransforms(exportMode, events) {
...
//# sourceMappingURL=GraphQLTelemetryExporter.js.map