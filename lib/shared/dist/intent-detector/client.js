...
export class SourcegraphIntentDetectorClient {
    completionsClient;
    constructor(completionsClient) {
    ...
    buildInitialTranscript(options) {
    ...
    buildExampleTranscript(options) {
        const messages = options.flatMap(({ id, examplePrompts }) => examplePrompts.flatMap(example => [
            {
                speaker: 'human',
                text: example,
            },
    ...
    async classifyIntentFromOptions(input, options, fallback) {
        ...
        for await (const message of stream) {
            switch (message.type) {
                ...
                case 'error': {
                    console.error(`Error detecting intent: Status code ${message.statusCode}: ${message.error.message}`);
                    return fallback;
                }
            }
        }
