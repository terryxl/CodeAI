...
import { displayPath, displayPathBasename, displayPathDirname, setDisplayPathEnvInfo, uriHasPrefix, } from './displayPath';
...
function withEnvInfo(envInfo, fn) {
    const prev = setDisplayPathEnvInfo(envInfo);
    try {
        return fn();
    }
    finally {
        setDisplayPathEnvInfo(prev);
    }
}
describe('displayPath', () => {
    function displayPathWithEnvInfo(location, envInfo) {
        return withEnvInfo(envInfo, () => displayPath(location));
    }
    for (const { name, tests: { nonWindows, windows, all }, } of DISPLAY_PATH_TEST_CASES) {
        function runTestCases(envInfo, cases) {
            test(name, () => {
        ...
        if (nonWindows) {
            ...
            describe.skipIf(isWindows())('nonWindows', () => runTestCases({ ...nonWindows.envInfo, isWindows: false }, nonWindows.cases));
        }
        if (windows) {
            describe('windows', () => runTestCases({ ...windows.envInfo, isWindows: true }, windows.cases));
        }
        if (all) {
            describe.skipIf(isWindows())('all nonWindows', () => runTestCases({ ...all.envInfo, isWindows: false }, all.cases));
            describe('all windows', () => runTestCases({ ...all.envInfo, isWindows: true }, all.cases));
        }
    }
});
describe('displayPathDirname', () => {
    function displayPathDirnameWithEnvInfo(location, envInfo) {
        return withEnvInfo(envInfo, () => displayPathDirname(location));
    }
    describe('nonWindows', () => {
        test('no workspace folders', () => {
            const envInfo = { isWindows: false, workspaceFolders: [] };
            if (!isWindows()) {
            ...
            expect(displayPathDirnameWithEnvInfo(URI.parse('https://example.com/foo/bar.ts'), envInfo)).toBe('https://example.com/foo');
        });
        test('1 workspace folder', () => {
            ...
            expect(displayPathDirnameWithEnvInfo(URI.parse('https://example.com/a/b'), envInfo)).toBe('https://example.com/a');
        });
        test('2 workspace folders', () => {
            ...
            if (!isWindows()) {
                // Skip on Windows because there is still some magic in the URI library for file: URIs
                // on Windows.
                expect(displayPathDirnameWithEnvInfo(URI.parse('file:///workspace1/foo.ts'), envInfo)).toBe('workspace1');
                expect(displayPathDirnameWithEnvInfo(URI.parse('file:///workspace2/foo.ts'), envInfo)).toBe('workspace2');
                expect(displayPathDirnameWithEnvInfo(URI.parse('file:///workspace1/foo/bar.ts'), envInfo)).toBe('workspace1/foo');
                expect(displayPathDirnameWithEnvInfo(URI.parse('file:///workspace2/foo/bar.ts'), envInfo)).toBe('workspace2/foo');
                expect(displayPathDirnameWithEnvInfo(URI.parse('file:///other/foo.ts'), envInfo)).toBe('/other');
            }
            expect(displayPathDirnameWithEnvInfo(URI.parse('https://example.com/a/b'), envInfo)).toBe('https://example.com/a');
        });
    ...
    describe('windows', () => {
        test('no workspace folders', () => {
            const envInfo = { isWindows: true, workspaceFolders: [] };
        ...
        test('2 workspace folders', () => {
            ...
            expect(displayPathDirnameWithEnvInfo(windowsFileURI('C:\\workspace1\\foo\\bar.ts'), envInfo)).toBe('workspace1\\foo');
            expect(displayPathDirnameWithEnvInfo(windowsFileURI('C:\\workspace2\\foo\\bar.ts'), envInfo)).toBe('workspace2\\foo');
            expect(displayPathDirnameWithEnvInfo(windowsFileURI('C:\\other\\foo.ts'), envInfo)).toBe('c:\\other');
            expect(displayPathDirnameWithEnvInfo(URI.parse('https://example.com/a/b'), envInfo)).toBe('https://example.com/a');
        });
    });
...
describe('displayPathBasename', () => {
    function displayPathBasenameWithEnvInfo(location, envInfo) {
...
describe('uriHasPrefix', () => {
    test('same url', () => expect(uriHasPrefix(URI.parse('https://example.com/a/b'), URI.parse('https://example.com/a/b'), false)).toBe(true));
    test('https path prefix', () => {
        expect(uriHasPrefix(URI.parse('https://example.com/a/b'), URI.parse('https://example.com/a'), false)).toBe(true);
        expect(uriHasPrefix(URI.parse('https://example.com/a/b'), URI.parse('other://example.com/a'), false)).toBe(false);
        expect(uriHasPrefix(URI.parse('https://example.com/a/b'), URI.parse('https://example.com/a/'), false)).toBe(true);
        expect(uriHasPrefix(URI.parse('https://example.com/a'), URI.parse('https://example.com/a/'), false)).toBe(true);
        expect(uriHasPrefix(URI.parse('https://example.com/a-b'), URI.parse('https://example.com/a'), false)).toBe(false);
    });
...
//# sourceMappingURL=displayPath.test.js.map