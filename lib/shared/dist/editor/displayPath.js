 ...
 */
export function displayPath(location) {
 ...
 */
export function displayPathDirname(location) {
 ...
 */
export function displayPathBasename(location) {
    const envInfo = checkEnvInfo();
    const result = _displayPath(location, envInfo);
    // File path.
    if (typeof result === 'string') {
 ...
 */
export function displayPathWithoutWorkspaceFolderPrefix(location) {
    const result = _displayPath(location, checkEnvInfo(), false);
    return typeof result === 'string' ? result : result.toString();
}
function checkEnvInfo() {
    if (!envInfo) {
        throw new Error('no environment info for displayPath function (call setDisplayPathEnvInfo; see displayPath docstring for more info)');
    }
    return envInfo;
}
    ...
    for (const folder of workspaceFolders) {
        if (uriHasPrefix(uri, folder, isWindows)) {
            ...
            const workspaceDisplayPrefix = includeWorkspaceFolder
                ...
                : '';
            return fixPathSep(workspaceDisplayPrefix + uri.path.slice(workspacePrefix.length + 1), isWindows, uri.scheme);
        }
    }
    if (uri.scheme === 'file') {
...
export function uriHasPrefix(uri, prefix, isWindows) {
    ...
    // from).
    const uriPath = isWindows && uri.scheme === 'file'
        ? uri.path.slice(0, 2).toUpperCase() + uri.path.slice(2)
        : uri.path;
    const prefixPath = isWindows && prefix.scheme === 'file'
 ...
 */
export function setDisplayPathEnvInfo(newEnvInfo) {
    const prev = envInfo;
    envInfo = newEnvInfo;
    return prev;
}
//# sourceMappingURL=displayPath.js.map