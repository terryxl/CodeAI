    ...
    private nestSpans(spans: ReadableSpan[]): FormattedSpan[] {
        ...
        for (const span of spans) {
            ...
            if (parentSpanId) {
                ...
                formattedSpans[parentSpanId] = parentSpan as FormattedSpan;
            } else {
            ...
            if (hasEvents || hasChildren) {
                ...
                for (const eventOrSpan of sortByHrTime(eventsWithChildren)) {
                    if (isFormattedSpan(eventOrSpan)) {
                        ...
                        console.log(
                            `%c${name}%c${attributesString}${textString}`,
                            ...logStyles
                        );
                    }
                }
...
function sortByHrTime<T extends Pick<ReadableSpan, "startTime">>(
    spans: T[]
): T[] {
