...
import { AzuerPrompter } from "./PromptAzure";
...
export class SimpleChatPanelProvider implements vscode.Disposable, ChatSession {
    ...
    private readonly source: ChatEventSource;
    private readonly remoteSearch: RemoteSearch | null;
    private readonly repoPicker: RemoteRepoPicker | null;
    public handleUserMessageFn: typeof this.handleUserMessageSubmission;
    ...
    constructor({
        ...
        this.source = source;
        this.model = chatModel.getModel(authProvider, models);
        this.chatModel = new SimpleChatModel(this.model.model);
        ...
        if (this.remoteSearch) {
            this.disposables.push(
                ...
                this.codebaseStatusProvider.onDidChangeStatus(async () => {
                    const codebase =
                        await this.codebaseStatusProvider.currentCodebase();
    ...
    private async onDidReceiveMessage(message: WebviewMessage): Promise<void> {
        switch (message.command) {
            ...
            case "initialized":
                ...
                break;
            case "submit": {
                await this.handleUserMessageFn(
                    uuid.v4(),
                    message.text,
                    message.submitType,
            ...
            async (span): Promise<void> => {
                ...
                tracer.startActiveSpan(
                    ...
                    async (firstTokenSpan): Promise<void> => {
                        ...
                        this.chatModel.addHumanMessage(
                            { text: promptText },
                            displayText
                        );
                        await this.saveSession({
                        ...
                        const userContextItems =
                            await contextFilesToContextItems(
                                this.editor,
                                userContextFiles || [],
                                true
                            );
    ...
    public async handleUserMessageSubmissionForAzure(
        requestID: string,
        inputText: string,
        submitType: ChatSubmitType,
        userContextFiles?: ContextFile[],
    ...
    private async handleEdit(
        ...
        try {
            ...
            return await this.handleUserMessageFn(
                requestID,
                text,
                "user",
                contextFiles,
    ...
    private postChatModels(): void {
        ...
        const models = ModelProvider.get(
            ModelUsage.Chat,
            authStatus.endpoint,
     ...
     */
    private async buildPrompt<T = Message>(
