...
const releaseType = process.env.CODY_RELEASE_TYPE || ReleaseType.Stable;
function validateReleaseType(
    releaseType: string | undefined
): asserts releaseType is ReleaseType {
    if (
        !releaseType ||
        !Object.values(ReleaseType).includes(releaseType as ReleaseType)
    ) {
        console.error(
            `Invalid release type ${JSON.stringify(
                releaseType
...
const customDefaultSettingsFile =
    process.env.CODY_RELEASE_CUSTOM_DEFAULT_SETTINGS_FILE;
if (customDefaultSettingsFile) {
    ...
    console.log(
        `Applying custom default settings from ${customDefaultSettingsFile}`
    );

    const configurationProperties =
        packageJSON.contributes.configuration.properties;

    const missingSettings: any = [];
    for (const [name, value] of Object.entries(settingsDefaults)) {
        const foundProperty = configurationProperties[name] as { default: any };
        if (foundProperty) {
            console.log(
                `\t- Setting custom default setting for "${name}" with value "${value}"`
            );
...
const version =
    releaseType === ReleaseType.Insiders ? insidersVersion : packageJSONVersion;
