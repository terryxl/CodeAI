...
helpers_1.test.extend({
    ...
    await (0, test_1.expect)(page.getByText('Authentication failed.')).not.toBeVisible();
    ...
    await page.getByRole('combobox', { name: 'input' }).press('Enter');
    await (0, test_1.expect)(page.getByRole('alert').getByText('Authentication failed.')).toBeVisible();
    await sidebar.getByRole('button', { name: 'Sign In to Your Enterprise Instance' }).click();
    await page.getByRole('option', { name: 'Sign In with URL and Access Token' }).click();
    await page.getByRole('combobox', { name: 'input' }).fill(mock_server_1.SERVER_URL);
    await page.getByRole('combobox', { name: 'input' }).press('Enter');
    ...
    await page.getByRole('combobox', { name: 'input' }).press('Enter');
    // Sign out.
    await (0, helpers_1.signOut)(page);
    const sidebarFrame = page.frameLocator('iframe.webview').frameLocator('iframe').first();
    await (0, test_1.expect)(sidebarFrame.getByRole('button', { name: 'Sign In to Your Enterprise Instance' })).toBeVisible();
    ...
    await (0, test_1.expect)(page.getByRole('heading', { name: 'Cody: Chat' })).toBeVisible();
