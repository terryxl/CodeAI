...
function getConfiguration(config = vscode.workspace.getConfiguration()) {
    ...
    return {
        ...
        useContext: config.get(configuration_keys_1.CONFIG_KEY.useContext) || 'embeddings',
...
const getFullConfig = async () => {
    ...
    const isTesting = process.env.CODY_TESTING === 'true';
    const serverEndpoint = LocalStorageProvider_1.localStorage?.getEndpoint() || (isTesting ? 'http://localhost:49300/' : cody_shared_1.DOTCOM_URL.href);
