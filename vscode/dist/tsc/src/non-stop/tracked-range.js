...
exports.updateFixedRange = exports.updateRange = exports.updateRangeMultipleChanges = void 0;
 ...
 */
function updateRangeMultipleChanges(range, changes, options = {}, rangeUpdater = updateRange) {
    changes.sort((a, b) => (b.range.start.isBefore(a.range.start) ? -1 : 1));
    for (let i = 0; i < changes.length - 1; i++) {
        console.assert(changes[i].range.start.isAfterOrEqual(changes[i + 1].range.end), 'vscode edit model assumption incorrect');
    }
...
exports.updateRangeMultipleChanges = updateRangeMultipleChanges;
...
function updateRange(range, change, options = {}) {
    ...
    if (options.supportRangeAffix &&
        change.range.start.isEqual(range.end) &&
        change.text.trim().length > 0) {
        return new vscode.Range(range.start, change.range.end.translate(change.range.start.line - change.range.end.line + insertedLineBreaks, change.range.end.line === range.end.line
    ...
    if (options.supportRangeAffix &&
        ...
        change.text.trim().length > 0) {
        return new vscode.Range(change.range.start, range.end.translate(change.range.start.line - change.range.end.line + insertedLineBreaks, change.range.end.line === range.end.line
            ? insertedLastLine -
    ...
    if (change.range.end.isBeforeOrEqual(range.start)) {
        range = range.with(range.start.translate(change.range.start.line - change.range.end.line + insertedLineBreaks, change.range.end.line === range.start.line
            ? insertedLastLine +
                -change.range.end.character +
                (insertedLineBreaks === 0 ? change.range.start.character : 0)
            : 0), range.end.translate(change.range.start.line - change.range.end.line + insertedLineBreaks, change.range.end.line === range.end.line
            ? insertedLastLine -
                change.range.end.character +
                (insertedLineBreaks === 0 ? change.range.start.character : 0)
            : 0));
    ...
    // ...around
    else if (change.range.start.isBeforeOrEqual(range.start) &&
        change.range.end.isAfterOrEqual(range.end)) {
        return new vscode.Range(change.range.start, change.range.start);
    }
    // ...within
    else if (change.range.start.isAfterOrEqual(range.start) &&
        change.range.end.isBeforeOrEqual(range.end)) {
        range = range.with(range.start, range.end.translate(change.range.start.line - change.range.end.line + insertedLineBreaks, change.range.end.line === range.end.line
            ? change.range.start.character - change.range.end.character + insertedLastLine
            : 0));
    }
    // ...overlapping start
        ...
        change.range.end.translate(change.range.start.line - change.range.end.line, change.range.start.character - change.range.end.character + insertedLastLine), 
        // Adjust the end of the decoration for the range deletion
        range.end.translate(change.range.start.line - change.range.end.line, change.range.end.line === range.end.line
            ? change.range.start.character - change.range.end.character + insertedLastLine
            : 0));
    }
    // ...overlapping end
    else {
        range = range.with(range.start, 
 ...
 */
function updateFixedRange(range, change) {
    const lines = change.text.split(/\r\n|\r|\n/m);
    const insertedLastLine = lines.at(-1)?.length;
    if (insertedLastLine === undefined) {
        throw new TypeError('unreachable'); // Any string .split produces a non-empty array.
...
exports.updateFixedRange = updateFixedRange;
