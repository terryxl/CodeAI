...
exports.FixupController = void 0;
...
const provider_1 = require("./codelenses/provider");
const FixupContentStore_1 = require("./FixupContentStore");
const FixupDecorator_1 = require("./FixupDecorator");
const FixupDocumentEditObserver_1 = require("./FixupDocumentEditObserver");
const FixupFileObserver_1 = require("./FixupFileObserver");
const FixupScheduler_1 = require("./FixupScheduler");
const FixupTask_1 = require("./FixupTask");
const utils_2 = require("./utils");
const get_input_1 = require("../edit/input/get-input");
const constants_1 = require("./codelenses/constants");
...
class FixupController {
    ...
    scheduler = new FixupScheduler_1.FixupScheduler(10);
    decorator = new FixupDecorator_1.FixupDecorator();
    codelenses = new provider_1.FixupCodeLenses(this);
    contentStore = new FixupContentStore_1.ContentProvider();
    _disposables = [];
    constructor(authProvider) {
        this.authProvider = authProvider;
        // Register commands
        this._disposables.push(vscode.workspace.registerTextDocumentContentProvider('cody-fixup', this.contentStore), vscode.commands.registerCommand('cody.fixup.codelens.cancel', id => {
            telemetry_1.telemetryService.log('CodyVSCodeExtension:fixup:codeLens:clicked', {
                op: 'cancel',
                hasV2Event: true,
            });
            telemetry_v2_1.telemetryRecorder.recordEvent('cody.fixup.codeLens', 'cancel');
            ...
            telemetry_v2_1.telemetryRecorder.recordEvent('cody.fixup.codeLens', 'diff');
            return this.diff(id);
            ...
            telemetry_v2_1.telemetryRecorder.recordEvent('cody.fixup.codeLens', 'retry');
            ...
            telemetry_v2_1.telemetryRecorder.recordEvent('cody.fixup.codeLens', 'undo');
            return this.undo(id);
            ...
            telemetry_v2_1.telemetryRecorder.recordEvent('cody.fixup.codeLens', 'accept');
            ...
            telemetry_v2_1.telemetryRecorder.recordEvent('cody.fixup.codeLens', 'showError');
            ...
            telemetry_v2_1.telemetryRecorder.recordEvent('cody.fixup.codeLens', 'skipFormatting');
            return this.skipFormatting(id);
        }), vscode.commands.registerCommand('cody.fixup.cancelNearest', () => {
            const nearestTask = this.getNearestTask({ filter: { states: constants_1.CANCELABLE_TASK_STATES } });
            if (!nearestTask) {
                return;
            }
            ...
            const nearestTask = this.getNearestTask({ filter: { states: constants_1.ACTIONABLE_TASK_STATES } });
            if (!nearestTask) {
                return;
            }
        ...
        this.files = new FixupFileObserver_1.FixupFileObserver();
        ...
        // Observe file edits
        this.editObserver = new FixupDocumentEditObserver_1.FixupDocumentEditObserver(this);
        this._disposables.push(vscode.workspace.onDidChangeTextDocument(this.editObserver.textDocumentChanged.bind(this.editObserver)));
        ...
        const autoSaveSetting = vscode.workspace.getConfiguration('files').get('autoSave');
        if (autoSaveSetting === 'off' || autoSaveSetting === 'onWindowChange') {
            this._disposables.push(vscode.workspace.onDidSaveTextDocument(({ uri }) => {
    ...
    async promptUserForTask(document, range, expandedRange, mode, model, intent, contextMessages, source) {
        const input = await (0, get_input_1.getInput)(document, this.authProvider, {
            initialRange: range,
        ...
        const task = this.createTask(document, input.instruction, input.userContextFiles, input.range, input.intent, mode, input.model, source, contextMessages);
        // Return focus to the editor
        void vscode.window.showTextDocument(document);
        return task;
    }
    async createTask(document, instruction, userContextFiles, selectionRange, intent, mode, model, source, contextMessages, destinationFile) {
        const fixupFile = this.files.forUri(document.uri);
        const task = new FixupTask_1.FixupTask(fixupFile, instruction, userContextFiles, intent, selectionRange, mode, model, source, contextMessages, destinationFile);
        this.tasks.set(task.id, task);
        const state = task.intent === 'test' ? utils_2.CodyTaskState.pending : utils_2.CodyTaskState.working;
        this.setTaskState(task, state);
        return task;
    }
    // Apply single fixup from task ID. Public for testing.
    async apply(id) {
        (0, log_1.logDebug)('FixupController:apply', 'applying', { verbose: { id } });
    ...
    // diff.
    applicableDiffOrRespin(task, document) {
        if (task.state !== utils_2.CodyTaskState.applying && task.state !== utils_2.CodyTaskState.applied) {
        ...
        if (task.replacement !== undefined && bufferText !== diff?.bufferText) {
            ...
            task.diff = diff = (0, diff_1.computeDiff)(task.original, task.replacement, bufferText, task.selectionRange.start);
    ...
    scheduleRespin(task) {
        ...
        if (task.spinCount >= MAX_SPIN_COUNT_PER_TASK) {
            telemetry_1.telemetryService.log('CodyVSCodeExtension:fixup:respin', {
                count: task.spinCount,
                hasV2Event: true,
            });
            telemetry_v2_1.telemetryRecorder.recordEvent('cody.fixup.respin', 'scheduled', {
                metadata: { spinCount: task.spinCount },
            });
            this.error(task.id, new Error(`Cody tried ${task.spinCount} times but failed to edit the file`));
            return;
        }
        void vscode.window.showInformationMessage('Cody will rewrite to include your changes');
        this.setTaskState(task, utils_2.CodyTaskState.working);
    }
    logTaskCompletion(task, editOk) {
        if (!editOk) {
            telemetry_1.telemetryService.log('CodyVSCodeExtension:fixup:apply:failed', undefined, {
                hasV2Event: true,
            });
            telemetry_v2_1.telemetryRecorder.recordEvent('cody.fixup.apply', 'failed');
        ...
        const codeCount = (0, code_count_1.countCode)(task.replacement.trim());
        const source = task.source;
        telemetry_1.telemetryService.log('CodyVSCodeExtension:fixup:applied', { ...codeCount, source }, { hasV2Event: true });
        telemetry_v2_1.telemetryRecorder.recordEvent('cody.fixup.apply', 'succeeded', {
            metadata: {
    ...
    async streamTask(task, state) {
        if (task.state !== utils_2.CodyTaskState.inserting) {
            return;
        }
        let edit;
        let document;
        const visibleEditor = vscode.window.visibleTextEditors.find(editor => editor.document.uri === task.fixupFile.uri);
        if (visibleEditor) {
        ...
        if (state === 'complete') {
            ...
            this.setTaskState(task, utils_2.CodyTaskState.formatting);
            await new Promise((resolve, reject) => {
                task.formattingResolver = resolve;
                this.formatEdit(visibleEditor ? visibleEditor.edit.bind(this) : new vscode.WorkspaceEdit(), document, task, formatEditOptions)
                    .then(resolve)
                    .catch(reject)
                    .finally(() => {
            ...
            this.setTaskState(task, utils_2.CodyTaskState.applied);
        ...
        if (editOk) {
            ...
            task.selectionRange = task.selectionRange.with(task.selectionRange.start, task.selectionRange.end.translate({
    ...
    async applyTask(task) {
        if (task.state !== utils_2.CodyTaskState.applying) {
            return;
        }
        let edit;
        let document;
    ...
    async replaceEdit(edit, diff, task, options) {
        (0, log_1.logDebug)('FixupController:edit', 'replacing ');
        if (edit instanceof vscode.WorkspaceEdit) {
            for (const diffEdit of diff.edits) {
                edit.replace(task.fixupFile.uri, new vscode.Range(new vscode.Position(diffEdit.range.start.line, diffEdit.range.start.character), new vscode.Position(diffEdit.range.end.line, diffEdit.range.end.character)), diffEdit.text);
            }
            return vscode.workspace.applyEdit(edit);
        }
        return edit(editBuilder => {
            for (const diffEdit of diff.edits) {
                editBuilder.replace(new vscode.Range(new vscode.Position(diffEdit.range.start.line, diffEdit.range.start.character), new vscode.Position(diffEdit.range.end.line, diffEdit.range.end.character)), diffEdit.text);
            }
        }, options);
    }
    // Insert edit returned by Cody at task selection range
    async insertEdit(edit, document, task, options) {
        (0, log_1.logDebug)('FixupController:edit', 'inserting');
    ...
    async formatEdit(edit, document, task, options) {
        ...
        (0, log_1.logDebug)('FixupController:edit', 'formatting');
        if (edit instanceof vscode.WorkspaceEdit) {
    ...
    async notifyTaskComplete(task) {
        ...
        const result = await vscode.window.showInformationMessage(`Edit applied to ${(0, cody_shared_1.displayPathBasename)(task.fixupFile.uri)}`, showChangesButton);
    ...
    cancelTask(task) {
        this.setTaskState(task, task.state === utils_2.CodyTaskState.error ? utils_2.CodyTaskState.error : utils_2.CodyTaskState.finished);
        this.discard(task);
    }
    accept(id) {
        const task = this.tasks.get(id);
        if (!task || task.state !== utils_2.CodyTaskState.applied) {
            return;
        }
        this.setTaskState(task, utils_2.CodyTaskState.finished);
        this.discard(task);
    }
    async undo(id) {
        const task = this.tasks.get(id);
     ...
     */
    async undoTask(task) {
        if (task.state !== utils_2.CodyTaskState.applied) {
            return;
        }
        ...
        if (!editOk) {
            telemetry_1.telemetryService.log('CodyVSCodeExtension:fixup:revert:failed', {
                hasV2Event: true,
            });
            telemetry_v2_1.telemetryRecorder.recordEvent('cody.fixup.revert', 'failed');
            return;
        }
        const tokenCount = (0, code_count_1.countCode)(replacementText);
        telemetry_1.telemetryService.log('CodyVSCodeExtension:fixup:reverted', tokenCount, {
            hasV2Event: true,
        });
        telemetry_v2_1.telemetryRecorder.recordEvent('cody.fixup.reverted', 'clicked', {
            metadata: tokenCount,
        });
        this.setTaskState(task, utils_2.CodyTaskState.finished);
    }
    error(id, error) {
        ...
        task.error = error;
        this.setTaskState(task, utils_2.CodyTaskState.error);
    }
    showError(id) {
        const task = this.tasks.get(id);
        if (!task?.error) {
    ...
    async didReceiveFixupInsertion(id, text, state) {
        const task = this.tasks.get(id);
        if (!task) {
            return;
        }
        if (task.state !== utils_2.CodyTaskState.inserting) {
            this.setTaskState(task, utils_2.CodyTaskState.inserting);
        }
    ...
    async didReceiveFixupText(id, text, state) {
        const task = this.tasks.get(id);
        if (!task) {
            return Promise.resolve();
        }
        if (task.state !== utils_2.CodyTaskState.working) {
        ...
        switch (state) {
            ...
            case 'complete':
                ...
                this.setTaskState(task, utils_2.CodyTaskState.applying);
                break;
        }
     ...
     */
    async didReceiveNewFileRequest(id, newFileUri) {
        const task = this.tasks.get(id);
        if (!task) {
            return;
        }
        if (task.fixupFile.uri.toString() === newFileUri.toString()) {
            return this.setTaskState(task, utils_2.CodyTaskState.working);
        }
    ...
    textDidChange(task) {
        ...
        if (task.state === utils_2.CodyTaskState.finished) {
            this.needsDiffUpdate_.delete(task);
        }
    ...
    needsDiffUpdate_ = new Set();
    // Files where the editor wasn't visible and we have delayed computing diffs
    // for tasks.
    needsEditor_ = new Set();
    didChangeVisibleTextEditors(editors) {
        const editorsByFile = new Map();
    ...
    updateDiffs() {
        while (this.needsDiffUpdate_.size) {
            ...
            task.diff = (0, diff_1.computeDiff)(task.original, `${botText}${newLine}`, bufferText, task.selectionRange.start);
            this.didUpdateDiff(task);
        }
    }
    didUpdateDiff(task) {
    ...
    async diff(id) {
        ...
        await vscode.commands.executeCommand('vscode.diff', tempDocUri, task.fixupFile.uri, `Cody Edit Diff View - ${task.id}`, {
            preview: true,
            preserveFocus: false,
    ...
    setTaskState(task, state) {
        ...
        if (task.state === utils_2.CodyTaskState.pending && task.destinationFile) {
            void this.didReceiveNewFileRequest(task.id, task.destinationFile);
            return;
        }
        if (oldState !== utils_2.CodyTaskState.working && task.state === utils_2.CodyTaskState.working) {
            task.spinCount++;
        }
        if (task.state === utils_2.CodyTaskState.finished) {
            this.discard(task);
    ...
    getNearestTask({ filter }) {
        ...
        const closestTask = this.tasksForFile(fixupFile)
            ...
            .sort((a, b) => (0, utils_2.getMinimumDistanceToRangeBoundary)(position, a.selectionRange) -
            (0, utils_2.getMinimumDistanceToRangeBoundary)(position, b.selectionRange))[0];
        return closestTask;
    }
    reset() {
        this.tasks = new Map();
...
exports.FixupController = FixupController;
