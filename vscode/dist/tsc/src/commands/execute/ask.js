 ...
 */
const executeChat = async (args) => {
    const { chat, commands } = await cody_shared_1.ConfigFeaturesSingleton.getInstance().getConfigFeatures();
    const isCommand = (0, _1.isDefaultChatCommand)(args.source || '');
    ...
    if (isCommand && (0, active_editor_1.getEditor)()?.ignored) {
        void vscode.window.showErrorMessage('Cannot execute a command in an ignored file.');
        return undefined;
    }
    return vscode.commands.executeCommand('cody.action.chat', args);
};
