...
exports.findRangeByLine = exports.getSelectionAroundLine = exports.getDocumentSections = void 0;
const vscode = __importStar(require("vscode"));
const foldingRanges_1 = require("../../lsp/foldingRanges");
const isIndentationBasedFoldingRanges = vscode.workspace.getConfiguration().get('cody.experimental.foldingRanges', 'lsp') ===
    'indentation-based';
async function getDocumentSections(doc, 
// Optional overwrites to simplify testing
getFoldingRanges = defaultGetFoldingRanges, getSymbols = defaultGetSymbols) {
 ...
 */
async function getSelectionAroundLine(doc, line) {
...
exports.getSelectionAroundLine = getSelectionAroundLine;
/**
 ...
 */
function findRangeByLine(ranges, targetLine) {
    return ranges.find(range => range.start.line <= targetLine && range.end.line >= targetLine);
}
exports.findRangeByLine = findRangeByLine;
 ...
 */
async function getOutermostFoldingRanges(doc, ranges, getSymbols) {
    ...
    return outermostFoldingRanges
        ...
        .filter((r, i) => firstLines[i].includes('class') ||
 ...
 */
async function removeOutermostFoldingRanges(doc, foldingRanges, getSymbols) {
 ...
 */
function removeNestedFoldingRanges(ranges, isTextBased = false) {
    const filtered = isTextBased ? combineNeighborFoldingRanges(ranges) : ranges;
    return filtered.filter(cur => !filtered.some(next => next !== cur && next.start <= cur.start && next.end >= cur.end));
}
/**
 ...
 */
function combineNeighborFoldingRanges(ranges) {
 ...
 */
function foldingRangeToRange(doc, range) {
...
async function defaultGetFoldingRanges(uri) {
    ...
    if (isIndentationBasedFoldingRanges) {
        const provider = new foldingRanges_1.IndentationBasedFoldingRangeProvider();
