...
exports.getWordOccurrences = exports.bestJaccardMatches = void 0;
 ...
 */
function bestJaccardMatches(targetText, matchText, windowSize, maxMatches) {
    if (windowSize < 1) {
    ...
    const intersectionOccurrences = new Map();
    ...
    for (let i = 1; i <= lines.length - windowSize; i++) {
        ...
        const { windowIncrease, intersectionIncrease } = add(targetOccurrences, windowOccurrences, intersectionOccurrences, wordsForEachLine[i + windowSize - 1]);
...
exports.bestJaccardMatches = bestJaccardMatches;
function jaccardSimilarity(left, right, intersection) {
...
function getWordOccurrences(s) {
    const frequencyCounter = new Map();
    const words = wink_nlp_utils_1.default.string.tokenize0(s);
    const filteredWords = wink_nlp_utils_1.default.tokens.removeWords(words);
    const stems = wink_nlp_utils_1.default.tokens.stem(filteredWords);
    for (const stem of stems) {
        frequencyCounter.set(stem, (frequencyCounter.get(stem) || 0) + 1);
    }
    return frequencyCounter;
}
exports.getWordOccurrences = getWordOccurrences;
function sumWordCounts(words) {
    let count = 0;
...
function add(target, window, intersection, newLine) {
